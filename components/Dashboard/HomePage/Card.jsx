    ...
    <>
      <div
        ...
        <Image
          width={400}
          height={235}
          className={`w-[400px] absolute h-[235px] bg-cover rounded-lg select-none duration-200 transition-all ease-in ${isHovered[idx] ? "opacity-100" : "opacity-10"}`}
          src={project.image}
          alt={project.title}
        />

        <div
          className={`hidden lg:flex px-7 py-5 w-[400px] h-[235px] flex-col justify-between duration-200 transition-all ease-in ${isHovered[idx] ? "opacity-0" : "opacity-100"}`}>
